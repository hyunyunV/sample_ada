...
from Module.base import BaseEnsemble
...
from sklearn.utils.validation import _num_samples#------ 추가된 모듈들
...
class BaseWeightBoosting(BaseEnsemble, metaclass=ABCMeta):
    ...
    def fit2(self,X,y):
            ...
            for estimator in self.estimators_:
                all_proba.append(estimator.predict_proba(X))
    
            x0 = self.estimator_weights_
            #VV
            def g(x):
                return sum(x)-1
            def h(x):
    ...
    def decision_function(self, X):
        ...
        if self.algorithm == 'SAMME.R':
            ...
            pred = sum(_samme_proba(estimator, n_classes, X)
                       for estimator in self.estimators_)
        elif self.newfit == True :
            pred = sum(self.estimators_[i].predict_proba(X)*self.new_weight[i] for i in range(len(self.new_weight)))
        else:  # self.algorithm == "SAMME"
            pred = sum((estimator.predict(X) == classes).T * w # 수정..#VV
